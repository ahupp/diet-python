...
impl Transformer for ClassDefRewriter {
    fn visit_stmt(&self, stmt: &mut Stmt) {
        if let Stmt::ClassDef(ast::StmtClassDef { name, body, arguments, .. }) = stmt {
            ...
            ns_body.push(crate::py_stmt!("__class__ = None"));
            ns_body.push(crate::py_stmt!("def __class_factory__():\n    return __class__"));
            ns_body.push(crate::py_stmt!(
                "__classcell__ = __class_factory__.__closure__[0]"
            ));
            ns_body.push(crate::py_stmt!(
                "_dp_temp_ns[\"__classcell__\"] = _ns[\"__classcell__\"] = __classcell__"
            ));
                ...
                match stmt {
                    Stmt::Assign(ast::StmtAssign { targets, value, .. }) => {
                        if let [Expr::Name(ast::ExprName { id, .. })] = targets.as_slice() {
                            ns_body.push(crate::py_stmt!(
                                "{name:id} = _dp_temp_ns[{k1:expr}] = _ns[{k2:expr}] = {v:expr}",
                                name = id.as_str(),
                    ...
                    Stmt::FunctionDef(func_def) => {
                        ...
                        ns_body.push(crate::py_stmt!(
                            "{fn_name:id} = _dp_temp_ns[{k1:expr}] = _ns[{k2:expr}] = {mk_name:id}()",
                            fn_name = fn_name.as_str(),
            ...
            let classcell_stmt = crate::py_stmt!(
                "_dp_cell = ns.get('__classcell__')\nif _dp_cell is not None:\n    dp_intrinsics.set_classcell(_dp_cell, cls)"
            );
            ...
            let class_func = crate::py_stmt!(
                ...
                body = vec![bases_stmt, prepare_stmt, exec_stmt, cls_stmt, classcell_stmt, ret_stmt]
            );
