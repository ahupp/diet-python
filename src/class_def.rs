    ...
    fn visit_stmt(&self, stmt: &mut Stmt) {
        if let Stmt::ClassDef(ast::StmtClassDef { name, body, arguments, .. }) = stmt {
            ...
            let bases_stmt = crate::py_stmt!(
                "bases = types.resolve_bases({b:expr})",
                b = bases_tuple
            );
            ...
            let prepare_stmt = if has_kw {
                ...
                crate::py_stmt!(
                    "meta, ns, kwds = types.prepare_class({n:expr}, bases, {d:expr})",
                    n = string_expr(&class_name),
                    d = dict_expr
                )
            } else {
                crate::py_stmt!(
                    "meta, ns, kwds = types.prepare_class({n:expr}, bases)",
                    n = string_expr(&class_name)
                )
            };
...
def _class_C():
    bases = types.resolve_bases(())
    meta, ns, kwds = types.prepare_class("C", bases)
    _dp_ns_C(ns)
    cls = meta("C", bases, ns)
    return cls
C = _class_C()
...
def _class_C():
    bases = types.resolve_bases((B,))
    meta, ns, kwds = types.prepare_class("C", bases, {"metaclass": Meta, "kw": 1})
    _dp_ns_C(ns)
