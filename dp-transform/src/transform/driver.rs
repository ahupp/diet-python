...
"#,
    ...
    fn scope_expr_for_child_named(&self, name: &str) -> Expr {
        if self.context().is_global_in_current_scope(name) {
            return py_expr!("None");
        }
        self.scope_expr_for_child()
    }
    ...
    fn rewrite_function_def_in_class(&mut self, mut func_def: ast::StmtFunctionDef) -> Vec<Stmt> {
        ...
        let scope_expr = self.scope_expr_for_child_named(original_name.as_str());
            ...
            match item {
                WorkItem::Process(stmt) => {
                    ...
                    match self.rewrite_stmt(stmt) {
                        Rewrite::Visit(mut stmts) => {
                            apply_stmt_range(&mut stmts, original_range);
            ...
            if let Some(src) = self.ctx.source_slice(range) {
                ...
                if normalized.len() >= 10 && normalized != default {
                    *expr = py_expr!(
                    ...
                    apply_expr_range(expr, original_range);
                    return;
                }
            }
        }
...
"#,
            ...
            Expr::Slice(ast::ExprSlice {
                ...
                py_expr!(
                    ...
                    lower = lower.map(|expr| *expr).unwrap_or_else(none_name),
                    upper = upper.map(|expr| *expr).unwrap_or_else(none_name),
                    step = step.map(|expr| *expr).unwrap_or_else(none_name),
                )
            }
        ...
        apply_expr_range(&mut rewritten, original_range);
        *expr = rewritten;
        self.visit_expr(expr);
    }
